		...
		if !traditionalScenario {
			...
			if empty(availableS) and empty(availableEB) and empty(availableCB){
				...
				if ds < maxDistancePackage_Scooter and ds<deb and ds<dcb and ds<dc {
					...
					mindistance <- dc;
					choice <- 5;
				} else if delb < maxDistancePackage_ElectricBike and delb<ds and delb<deb and delb<dcb and delb<dc {
					mindistance <- delb;
					choice <- 6;
				} else {
					choice <- 0;
...
species package control: fsm skills: [moving] {
	...
	state delivering_electricBike {
		...
		transition to: end when: electricBikeToDeliver.state != "in_use_packages" {
			target <- final_destination;
		}
		exit {
			if packageEventLog {ask logger { do logExitState; }}
	...
	state delivering_eBike {
		...
		transition to: end when: eBikeToDeliver.state != "in_use_packages" {
			target <- final_destination;
		}
		exit {
			if packageEventLog {ask logger { do logExitState; }}
	...
	state delivering_car {
		...
		transition to: end when: carToDeliver.state != "in_use_packages" {
			target <- final_destination;
		}
		exit {
			if packageEventLog {ask logger { do logExitState; }}
